...
struct QueryTabManagerRecordingTests {
    ...
    private func recorder() -> (QueryTabManager, () -> [Opened]) {
        ...
        var opened: [Opened] = []
        manager.onTableOpened = { name, schema, database, isPreview in
            opened.append(Opened(name: name, schema: schema, database: database, isPreview: isPreview))
        }
    ...
    func addTableTabReportsCommitted() throws {
        ...
        #expect(opened() == [Opened(name: "orders", schema: "sales", database: "shop", isPreview: false)])
    }
    ...
    func previewReportsAsPreview() throws {
        ...
        #expect(opened() == [Opened(name: "orders", schema: "sales", database: "shop", isPreview: true)])
    }
    ...
    func replaceReportsPreviewFlag() throws {
        ...
        _ = try manager.replaceTabContent(
            tableName: "orders", databaseName: "shop", schemaName: "sales", isPreview: true
        )
        #expect(opened == [Opened(name: "orders", schema: "sales", database: "shop", isPreview: true)])
    }
}

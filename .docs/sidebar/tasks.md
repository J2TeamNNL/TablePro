...
|---|---|---|---|---|
| Top-level Create Table button | Local checklist | — | DONE-2026-05-25 | `SidebarContainerViewController` thêm NSButton (+) cạnh `NSSearchField`. `SidebarContextMenu` bỏ "Create New Table...". Button disabled khi `safeModeLevel.blocksAllWrites`; observe qua `withObservationTracking`. |
...
| ER Diagram — context menu entry | Local checklist | [#1422](https://github.com/TableProApp/TablePro/pull/1422) | DONE-2026-05-29 | Entry vẫn còn trong `SidebarContextMenu`, đổi tên "View ER Diagram" → "Show ER Diagram" (verify HEAD `ee187978`, dòng 98-99). Review B2 (nghi bị gỡ) đã bác bỏ. |
...
Lần cuối check (2026-05-25, session sidebar-inspector-overflow):

- ✅ `FavoriteTablesStorage`, `SidebarTableOrdering` đã có. `TableRow` render trailing star toggle button.
- ✅ `RecentTablesStore` đã có; Recent section render ở đầu Tables sidebar (`SidebarView.recentSection`).
- ✅ Favorites tab còn **Tables** + **Queries** (không có Recent).
- ✅ `SidebarContextMenu` bỏ "Create New Table..." + Add/Remove Favorites.
- ✅ `SidebarContainerViewController` thêm "+" button cạnh search field.
- ✅ `SidebarContextMenuLogic.maintenanceGroupEnabled` helper thêm; `Menu("Maintenance")` dùng nó.
- ✅ #1353 SKIPPED theo quyết định của user.
- ❌ Inspector pane overflow chưa fix — xem [inspector-overflow.md](inspector-overflow.md) để implement riêng.

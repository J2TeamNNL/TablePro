...
internal final class SpannerOAuthServer: @unchecked Sendable {
    ...
    private static let logger = Logger(subsystem: "com.TablePro", category: "SpannerOAuthServer")
            ...
            let task = Task {
                ...
                self.lock.withLock {
                    if let cont = self.continuation {
                        ...
                        cont.resume(throwing: SpannerError.timeout("OAuth authorization timed out (2 minutes)"))
                    }
                }
                self.stop()
            }
    ...
    private func startListener() throws {
        ...
        listener.stateUpdateHandler = { [weak self] state in
            ...
            case .failed(let error):
                ...
                let authError = SpannerError.authFailed("OAuth server failed: \(error.localizedDescription)")
                self?.lock.withLock {
                    if let cont = self?.readyContinuation {
                        self?.readyContinuation = nil
    ...
    private func readRequest(from connection: NWConnection) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 8192) { [weak self] content, _, _, error in
            guard let self, let data = content, error == nil else {
                self?.resumeWithError(SpannerError.authFailed("Failed to read OAuth callback"))
                return
            }

            guard let requestString = String(data: data, encoding: .utf8) else {
                self.resumeWithError(SpannerError.authFailed("Invalid OAuth callback data"))
                return
            }
            ...
            if let code = self.extractAuthCode(from: requestString) {
                ...
                self.resumeWithError(SpannerError.authFailed("OAuth authorization denied: \(errorDesc)"))
            } else {

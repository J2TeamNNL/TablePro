...
//  SpannerDriverPlugin
//
//  Authentication providers for Google Cloud Spanner: Service Account JWT and
...
internal protocol SpannerAuthProvider: Sendable {
    func accessToken() async throws -> String
    var projectId: String { get }
}

internal enum SpannerError: Error, LocalizedError {
    case notConnected
    case authFailed(String)
    case apiError(code: Int, message: String, status: String?)
    case aborted(String)
    case sessionExpired
    ...
    var errorDescription: String? {
        ...
        case .apiError(_, let message, _):
            return message
        case .aborted(let detail):
            return detail
        case .sessionExpired:
            return String(localized: "Spanner session expired")
...
internal final class SpannerServiceAccountAuthProvider: @unchecked Sendable, SpannerAuthProvider {
    ...
    private static let logger = Logger(subsystem: "com.TablePro", category: "SpannerServiceAccountAuth")
    private static let tokenEndpoint = "https://oauth2.googleapis.com/token"
    private static let spannerScope = "https://www.googleapis.com/auth/cloud-platform"

    init(jsonData: Data, overrideProjectId: String?) throws {
        guard let json = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any] else {
            throw SpannerError.authFailed("Invalid service account JSON")
        }

        guard let email = json["client_email"] as? String else {
            throw SpannerError.authFailed("Missing client_email in service account JSON")
        }
        guard let key = json["private_key"] as? String else {
            throw SpannerError.authFailed("Missing private_key in service account JSON")
        }
        ...
        if self.projectId.isEmpty {
            throw SpannerError.authFailed("No project ID found in service account JSON or connection settings")
        }
    }
    ...
    private func createJWT() throws -> String {
        ...
        let claimsJson = """
        {"iss":"\(clientEmail)","scope":"\(Self.spannerScope)","aud":"\(Self.tokenEndpoint)","iat":\(iat),"exp":\(exp)}
        """
    ...
    private func signRS256(data: Data) throws -> Data {
        ...
        throw SpannerError.authFailed("Failed to create RSA key from private key PEM")
    }
    ...
    private func sign(data: Data, with key: SecKey) throws -> Data {
        ...
        guard let sig = SecKeyCreateSignature(
            ...
            throw SpannerError.authFailed("Failed to sign JWT: \(msg)")
        }
        return sig as Data
    }
    ...
    private func extractDERFromPEM(_ pem: String) throws -> Data {
        ...
        guard let data = Data(base64Encoded: base64String) else {
            throw SpannerError.authFailed("Failed to decode PEM private key")
        }
        return data
    }
    ...
    private func exchangeJWTForToken(_ jwt: String) async throws -> String {
        guard let url = URL(string: Self.tokenEndpoint) else {
            throw SpannerError.authFailed("Invalid token endpoint URL")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw SpannerError.authFailed("Invalid token response")
        }

        guard httpResponse.statusCode == 200 else {
            let body = String(data: data, encoding: .utf8) ?? "unknown"
            throw SpannerError.authFailed("Token exchange failed (HTTP \(httpResponse.statusCode)): \(body)")
        }

        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let accessToken = json["access_token"] as? String
        else {
            throw SpannerError.authFailed("Missing access_token in token response")
        }
...
internal final class SpannerADCAuthProvider: @unchecked Sendable, SpannerAuthProvider {
    let projectId: String
    private let lock = NSLock()
    private var _cachedToken: CachedToken?
    private var _delegate: SpannerAuthProvider?
    private let overrideProjectId: String?
    private static let logger = Logger(subsystem: "com.TablePro", category: "SpannerADCAuth")
    private static let tokenEndpoint = "https://oauth2.googleapis.com/token"
    ...
    init(overrideProjectId: String?) throws {
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw SpannerError.authFailed("Invalid application default credentials JSON")
        }

        let credType = json["type"] as? String ?? ""

        if credType == "service_account" {
            let delegate = try SpannerServiceAccountAuthProvider(jsonData: data, overrideProjectId: overrideProjectId)
            ...
            self._delegate = SpannerAuthorizedUserDelegate(
                clientId: clientId,
            ...
            guard let impersonationUrl = json["service_account_impersonation_url"] as? String,
                ...
                throw SpannerError.authFailed("Invalid impersonated_service_account credentials: missing required fields")
            }
            ...
            let sourceDelegate: SpannerAuthProvider
            switch sourceType {
            case "authorized_user":
                guard let clientId = sourceCredentials["client_id"] as? String,
                    ...
                    throw SpannerError.authFailed("Invalid source credentials for impersonated_service_account")
                }
                sourceDelegate = SpannerAuthorizedUserDelegate(
            ...
            case "service_account":
                guard let saData = try? JSONSerialization.data(withJSONObject: sourceCredentials) else {
                    throw SpannerError.authFailed("Invalid source service account credentials")
                }
                sourceDelegate = try SpannerServiceAccountAuthProvider(jsonData: saData, overrideProjectId: self.projectId)
            default:
                throw SpannerError.authFailed("Unsupported source credential type '\(sourceType)' in impersonated_service_account")
            }

            self._delegate = SpannerImpersonatedServiceAccountDelegate(
    ...
    func accessToken() async throws -> String {
        guard let delegate = lock.withLock({ _delegate }) else {
            throw SpannerError.authFailed("No credential delegate configured")
        }
...
private final class SpannerAuthorizedUserDelegate: @unchecked Sendable, SpannerAuthProvider {
    let projectId: String
    ...
    private func performTokenRefresh() async throws -> String {
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let body = String(data: data, encoding: .utf8) ?? "unknown"
            throw SpannerError.authFailed("Token refresh failed: \(body)")
        }

        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let accessToken = json["access_token"] as? String
        else {
            throw SpannerError.authFailed("Missing access_token in refresh response")
        }
...
private final class SpannerImpersonatedServiceAccountDelegate: @unchecked Sendable, SpannerAuthProvider {
    let projectId: String
    private let sourceProvider: SpannerAuthProvider
    private let impersonationUrl: String
    private let lock = NSLock()
    private var _cachedToken: CachedToken?
    private var _refreshTask: Task<String, Error>?

    init(sourceProvider: SpannerAuthProvider, impersonationUrl: String, projectId: String) {
    ...
    private func fetchImpersonatedToken() async throws -> String {
        ...
        guard let url = URL(string: impersonationUrl) else {
            throw SpannerError.authFailed("Invalid impersonation URL")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            ...
            throw SpannerError.authFailed("Service account impersonation failed: \(responseBody)")
        }
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw SpannerError.authFailed("Invalid impersonation response")
        }
...
internal final class SpannerOAuthBrowserAuthProvider: @unchecked Sendable, SpannerAuthProvider {
    let projectId: String
    private let clientId: String
    private let clientSecret: String
    ...
    private static let logger = Logger(subsystem: "com.TablePro", category: "SpannerOAuth")
    ...
    private func performBrowserAuthFlow() async throws -> String {
        ...
        guard let authUrl = components?.url else {
            server.stop()
            throw SpannerError.authFailed("Failed to build OAuth authorization URL")
        }
    ...
    private func exchangeAuthCode(_ code: String, redirectUri: String) async throws -> TokenResponse {
        guard let url = URL(string: Self.tokenEndpoint) else {
            throw SpannerError.authFailed("Invalid token endpoint")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let body = String(data: data, encoding: .utf8) ?? "unknown"
            throw SpannerError.authFailed("Token exchange failed: \(body)")
        }
...
internal final class SpannerEmulatorAuthProvider: Sendable, SpannerAuthProvider {
    let projectId: String

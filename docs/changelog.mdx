...
<Update label="August 21, 2026" description="v0.67.0">
  ### New Features
  ...
  - **Run one statement at a time**: The statement your cursor is in gets a faint background, and a run button appears in the gutter beside every statement. Both can be turned off in **Settings > Editor** (#2278)
  - **Statement navigation**: `Ctrl+Cmd+Left` and `Ctrl+Cmd+Right` step between statements, `Ctrl+Cmd+Enter` runs the current one and moves to the next, and `Option+Shift+Up` and `Option+Shift+Down` extend the selection a statement at a time (#2279)
  - **Named result tabs**: Each result is named after the statement that produced it, using a leading comment when you wrote one. Picking a result moves the editor cursor to the statement behind it (#2280)
  ...
  - **Back and Forward**: Step back through the tables you followed a foreign key into, from the toolbar or the View menu on `Ctrl+Cmd+[` and `Ctrl+Cmd+]`. Back restores the filters, sort, page and selected row you left behind, and stands down while a tab holds unsaved edits (#2316)
  - **Background notifications**: TablePro tells you when a slow query, import, export, backup or row save finishes while you are in another app, and clicking the notification selects the tab that ran it. Configure it in **Settings > Notifications** (#2265)
  - **Redis Sentinel and Redis Cluster**: Pick a Connection Mode in the connection form. Sentinel follows a failover to the new primary; Cluster reads the shard map, routes each command to the shard that owns its key, and follows MOVED and ASK redirects (#1021)
  - **MongoDB filters on nested fields**: Filter on paths like `customer.country` or `items.sku`, with **any element** and **same element** matching for fields inside an array, and values compared as the type the field actually holds (#2315)
  ...
  - Swipe left on a saved connection in the welcome window to reveal Edit and Delete (#2310)
  - Double-click a table, or press `Return` on it, to keep its tab instead of reusing the preview tab (#2235)
  - Execute All Statements is on the Execute button's menu, alongside `Cmd+Shift+Enter` (#2230)
  ...
  - Two tabs showing same-named objects from different databases carry the database in their names, so `orders` reads as `app.orders` and `staging.orders` (#2217)
  ...
  - The JSON and PHP tree filters both ignore accents, so `cafe` finds `café` (#2204)
  ...
  - Forward slashes in a nested value are no longer escaped as `\/`, so a path or MQTT topic like `device/state/up/122` reads the way it is stored (#2322)
  - A MongoDB filter TablePro could not translate quietly matched the whole collection; it now matches nothing. The raw filter row works, and Match Case applies to text fields only (#2315)
  ...
  - Import and export result alerts stay on screen instead of closing with the progress sheet behind them (#2314)
  - A failed import reports its line number, statement and the database's own message instead of only the rollback error (#2314)
  ...
  - A column whose cells carry an action button no longer cuts its value short, and Size to Fit measures the loaded page instead of about 30 rows of it (#2303)
  ...
  - An object list that cannot load says so and offers Retry, instead of retrying silently every 15 seconds with nothing on screen (#2294)
  ...
  - A `BEGIN ... END` body runs whole from the editor instead of being split at every semicolon inside it, and a semicolon inside a MySQL `#` comment is no longer read as a statement boundary (#2278)
  ...
  - DuckDB lists its databases, schemas, tables and columns on a Mac that cannot reach the internet. Every metadata query used to depend on a function fetched from the extension registry (#2285)
  ...
  - Dameng results are complete instead of stopping at the server's first 32KB batch. A 20,000 row table used to read back as 662 rows with nothing saying anything was missing (#2262)
  - Dameng tables with a `CLOB` or `BLOB` column read the right columns, rows and values, column comments show up, creating a table with an index or comment works, and connections recover after a query is stopped (#2262)
  ...
  - A plugin's own connection fields reach the connection form, so Redis gets its Key Separator field and its ElastiCache IAM section back (#1021)
  ...
  - JSON result mode shows the rows the grid is showing, in the same order, and resolves a selection to that same row. Rows marked for deletion no longer appear there (#2251)
  - `Cmd+F`, the Filters button and `Cmd+Option+F` behave correctly in JSON result mode, and Add Row, Duplicate Row, Paste and Delete no longer collapse the view to the row they touched (#2244)
  ...
  - Scrolling a result with hundreds of columns is no longer slow. A 500-column table carried about 18,000 live cells and laid all of them out on each frame (#1219)
  ...
  - **Open in New Tab** opens a second tab for a table that is already open, so you can browse it under two different filters (#2235)
  ...
  - Sorting a column keeps working after you hide or show columns, and **Hide All** hides every column of a MongoDB collection (#2234)
  ...
  - A table that exists in two databases can be opened in both, and Server Dashboard, Users & Roles, Query Insights, ER Diagram and a linked SQL file select the tab they already opened (#2217)
  ...
  - Switching connection no longer empties the SQL editor or kills its undo, syntax highlighting, shortcuts and Vim mode for the rest of the tab's life (#2236)
  ...
  - Open Quickly no longer draws a faint rectangular outline around its Liquid Glass surface on macOS 26 and later (#2321)

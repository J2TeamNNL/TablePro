...
There is no **SSH Tunnel** pane on SQLite, PGlite, libSQL, Beancount, BigQuery, Spanner, Cloudflare D1, DynamoDB, Elasticsearch, or Snowflake: each is reached over a local file, a loopback socket, or a vendor HTTP API.

...
Every provider except Cursor can call tools. Claude Agent passes them to the `claude` command instead, which approves them on its own terms, so per-card approval and the Assistant mode write floor do not reach it; the MCP server has to be on for that path. Local models depend on the model.

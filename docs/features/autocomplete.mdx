...
Each query tab completes against its own database and schema, not the one the object browser points at. Rebind a tab to another database and it suggests that database's tables. Schemas and dialects are cached per database, so two tabs on one connection but different databases keep separate caches.

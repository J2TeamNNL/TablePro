...
On servers that group objects by schema, an entry stands for a schema. On a single-file or single-database engine, such as SQLite, DuckDB, BigQuery, or Spanner, a connection has one entry, under the connection's name.

...
The Favorites tab in the sidebar has two sections: **Tables** for pinned tables and **Queries** for saved SQL. Both appear in the same sidebar tab so you can access them without switching views.

## Table Favorites

Right-click any table in the sidebar and choose **Add to Favorites**. The table:

- Gets a star badge (★) in the sidebar table list
- Moves to the top of its section
- Appears in the **Tables** group at the top of the Favorites tab

Double-click a table in the Favorites tab to open it. Right-click it to open the table, view its focused ER diagram, or remove it.

Favorites are stored by table name and sync through iCloud. If a favorited table name doesn't exist in the active connection, it is hidden.
